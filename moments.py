...
for q in [1.0,0.95,0.9,0.8, 0.75]:
	...
	vphiavg2 = inte.tplquad(wDF, 0, np.inf, lambda x: 0, lambda x: sqrt(2.*pot(x)),lambda x,y: 0, lambda x,y: sqrt(2.*pot(x)-y**2) )
	...
	sigr =  (vfact**2)*(16.0*pi*pi)*vphiavg[0]
	sigtheta = (vfact**2)*(16.0*pi*pi)*vphiavg2[0]
